window.onload = function () {
    // Задаем первые три div`а для: экрана, клавиатуры и подсказки по смене языка
    ...
    /* Дословно, keyboard объект включает в себя пять объектов row(0-4)
        каждый объект row содержит в себе объекты клавиш с четырьмя значениями
        эти значения есть пресеты, которые будут меняться в зависимости от
        того, активен ли shift и каков текущий язык(информация достается из печенек) */
    let keyboard = {
        row0: {
    ...
    let shift = 'shiftOff';
      ...
      // Делаем смену языка через печеньки
      /*Используем для поиcка в печеньках регулярные выражения regEx
        (^|;) от начала строки или ';' ([^;]*)(;|$) первое значене поле = НЕ начало строки и не ';'
        ну а второе значение либо конец строки либо ; */
